...
#define TRLTOG_DTYPE MPI_DOUBLE_PRECISION
...
#ifdef OMPGPU
  !$OMP TARGET DATA
  !$OMP  MAP(PRESENT,ALLOC:PGLAT) &
  !$OMP& MAP(TO:IGPTRSEND,INDOFF,KINDEX, LLUV,LLGP2,LLGP3A,LLGP3B,KPTRGP)
  !$OMP TARGET DATA IF(PRESENT(PGP))   MAP(FROM:PGP)
  !$OMP TARGET DATA IF(PRESENT(PGPUV)) MAP(FROM:PGPUV) MAP(TO:IUVLEVS,IUVPARS)
  !$OMP TARGET DATA IF(PRESENT(PGP2))  MAP(FROM:PGP2)  MAP(TO:IGP2PARS)
  !$OMP TARGET DATA IF(PRESENT(PGP3A)) MAP(FROM:PGP3A) MAP(TO:IGP3APARS,IGP3ALEVS)
  !$OMP TARGET DATA IF(PRESENT(PGP3B)) MAP(FROM:PGP3B) MAP(TO:IGP3BPARS, IGP3BLEVS)
#endif
  ...
  IF( IRECVTOT(MYPROC) > 0 )THEN
    ...
    IF (LLPGPONLY) THEN
      ...
      !$ACC&            LLINDER,KPTRGP,PGP,PGLAT,KINDEX)
#endif
      DO JBLK=1,NGPBLKS
        DO JFLD=1,IFLDS
          DO JKL=1,JK_MAX
    ...
    ELSE
      ...
      !$OMP&     MAP(TO:IGPTROFF,IFLDOFF,JK_MAX) COLLAPSE(3) &
      !$OMP&     SHARED(NGPBLKS,IFLDS,IGPTRSEND,MYSETW,INDOFF,MYPROC, &
      !$OMP&            IGPTROFF,LLUV,PGPUV,IUVLEVS,IUVPARS,LLGP2,PGP2,IGP2PARS, &
      !$OMP&            PGLAT,LLGP3A,PGP3A,IGP3ALEVS,IGP3APARS,LLGP3B,PGP3B,&
      !$OMP&            IGP3BLEVS,IGP3BPARS,KINDEX,IFLDOFF,JK_MAX)
#endif
#ifdef ACCGPU
      !$ACC PARALLEL LOOP DEFAULT(NONE) PRIVATE(IPOS,IFIRST,ILAST,IFLD,JK) &
      !$ACC&     COPYIN(IGPTROFF,IFLDOFF,JK_MAX) COLLAPSE(3) &
...
#ifdef OMPGPU
      !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO DEFAULT(NONE) PRIVATE(II) MAP(TO:ILEN) COLLAPSE(2) &
      !$OMP&  SHARED(ILEN,KF_FS,KINDEX,INDOFF,ISEND,ZCOMBUFS,INS,PGLAT)
#endif
#ifdef ACCGPU
      !$ACC PARALLEL LOOP DEFAULT(NONE) PRIVATE(II) COPYIN(ILEN) COLLAPSE(2) &
      !$ACC&  PRESENT(ILEN,KF_FS,KINDEX,INDOFF,ISEND,ZCOMBUFS,INS,PGLAT)
#endif
      DO JL=1,ILEN
        DO JFLD=1,KF_FS
...
!      IRECV_FLD_START = 1 !! INT(ZCOMBUFR(-1,INR),KIND=JPIM) !! is this always 1 ?
...
#ifdef OMPGPU
      !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO DEFAULT(NONE) PRIVATE(IFIRST,ILAST,JI,JK,IFLDT) &
      !$OMP&     MAP(TO:INR,KF_FS,IPOS,JPOS,IFLD,IFLDA,JK_MAX,IRECV_FLD_END) COLLAPSE(3) &
      !$OMP&     SHARED(NGPBLKS,IRECV_FLD_END,JK_MAX,IFLDA,IGPTRSEND,ISETW,IPOS,JPOS, &
      !$OMP&            LLINDER,PGP,KPTRGP,ZCOMBUFR,INR,LLPGPONLY,LLUV,PGPUV,IUVLEVS, &
      !$OMP&            IUVPARS,LLGP2,PGP2,IGP2PARS,LLGP3A,PGP3A,IGP3ALEVS,IGP3APARS, &
      !$OMP&            LLGP3B,PGP3B,IGP3BLEVS,IGP3BPARS)
#endif
#ifdef ACCGPU
      ...
      !$ACC&     PRESENT(NGPBLKS,IRECV_FLD_END,JK_MAX,IFLDA,IGPTRSEND,ISETW,IPOS,JPOS, &
      !$ACC&            LLINDER,PGP,KPTRGP,ZCOMBUFR,INR,LLPGPONLY,LLUV,PGPUV,IUVLEVS, &
...
#ifdef OMPGPU
    !$OMP END TARGET DATA   !! CREATE ZCOMBUFR
    !$OMP END TARGET DATA   !! CREATE ZCOMBUFS
#endif
#ifdef ACCGPU
    !$ACC END DATA
    !$ACC END DATA
