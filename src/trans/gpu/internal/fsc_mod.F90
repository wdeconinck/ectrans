...
IF(KF_UV > 0) THEN
#ifdef OMPGPU
  !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO DEFAULT(NONE) SHARED(ISTAGTF,IMEN,KF_UV,PUV,ZACHTE2)
#endif
#ifdef ACCGPU
...
IF(KF_SCDERS > 0)THEN
#ifdef OMPGPU
  !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO DEFAULT(NONE) SHARED(ISTAGTF,IMEN,KF_SCALARS,PNSDERS,ZACHTE2)
#endif
#ifdef ACCGPU
  !$ACC PARALLEL LOOP DEFAULT(NONE) &
...
IF(LUVDER)THEN
#ifdef OMPGPU
  !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO DEFAULT(NONE) PRIVATE(IR) SHARED(ISTAGTF,IMEN,KF_UV,PUVDERS,ZACHTE2,PUV)
#endif
#ifdef ACCGPU
...
IF(KF_SCDERS > 0)THEN
#ifdef OMPGPU
  !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO DEFAULT(NONE) PRIVATE(IR) SHARED(ISTAGTF,IMEN,KF_SCALARS,PEWDERS,ZACHTE2,PSCALAR)
#endif
#ifdef ACCGPU
  !$ACC PARALLEL LOOP PRIVATE(IR) DEFAULT(NONE) &
  !$ACC& COPYIN(KF_SCALARS,IMEN,ISTAGTF,ZACHTE2) &
